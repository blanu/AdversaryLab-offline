...
def testAdversary(adversary, model, positive, negative, feature):
...
#            print('Testing connection %s / %s / %s / %s' % (dataset, side, pcap, connfile))
            ...
            for sidename in ['positive', 'negative']:
              connid=dataset+'-'+side+'-'+pcap+'-'+connfile
...
#                  print(conn[direction+'Stats'].keys())
  ...
  save(fits, 'tests/'+adversary+'-'+feature)
...
for ad in ads:
  ...
  features=loadYaml('adversaries/'+ad+'/'+'features')
  parts=ad.split('-')
  positive=parts[0]
  negative=parts[1]
  for feature in features:
    ...
    if feature=='sequence':
      ...
      testAdversary(ad, model, positive, negative, feature)
